...
<section class="mx-auto max-w-4xl px-6 py-10">
	<header class="mb-10">
		...
		<h1 class="font-display mt-1 text-4xl font-semibold tracking-tight sm:text-5xl">Curriculum</h1>
	...
	<div class="space-y-3">
		{#each data.modules as module (module.slug)}
			...
			<a
				href={resolve(`/learn/${module.slug}`)}
			...
			>
				<span
					class="grid size-10 shrink-0 place-items-center rounded-lg border border-slate-200 bg-slate-50 font-mono text-sm font-semibold text-slate-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
				>
